...
void Show_Battery(void)
{
	...
	Battery_Capacity = ADValue*100/4000;
...
int First_Page_Clock(void)
{
	while(1)
	{
		...
		else if(KeyNum == 3)  //确认
		{
			OLED_Clear();
			OLED_Update();
			return clkflag;    //退出该页面，return就是结束这个循环，回到main，看你确定的是哪个按键，然后再跳转
		}
		else if(KeyNum == 4)
		{
...
void Show_SettingPage_UI(void)    //设置页ui
{
...
int SettingPage(void)
{
	while(1)
	{
		...
		else if (setflag_temp == 2){SetTime();}    //跳转到设置时间，然后刷新了ui
...
void Menu_Animation(void)
{
	...
	if(pre_selection >=1)
	{
		OLED_ShowImage(x_pre-48,16,32,32,Menu_Graph[pre_selection-1]);	  //图像变换
	}
	if(pre_selection >=2)
	{
...
void MenuToFunction(void)
{
	for(uint8_t i=0;i<=6;i++)
	{
		OLED_Clear();
			if(pre_selection >=1)
		{
			OLED_ShowImage(x_pre-48,16+8*i,32,32,Menu_Graph[pre_selection-1]);	  //图像变换
		}
...
{
	move_flag = 1;
	uint8_t DirectFlag = 2;//置1，移动到上一项；置2移动到下一项
	while(1)
	{
		KeyNum = Key_GetNum();
		...
		if(menu_flag_temp == 1){return 0;}     //返回
		else if (menu_flag_temp == 2){MenuToFunction();StopWatch();}    //跳转到设置时间，然后刷新了ui
...
uint8_t start_timing_flag;  //1表示开始计时；0表示停止计时
...
int StopWatch(void)
{
	while(1)
	{
		...
		if(stopwatch_flag_temp == 1){return 0;}     //返回
...
int LED(void)
{
	while(1)
	{
		...
		if(LED_flag_temp == 1){return 0;}     //返回
...
float roll_g,pitch_g,yaw_g;//陀螺仪结算的欧拉角
float roll_a,pitch_a;//加速度计结算的欧拉角  
...
int Game(void)
{
	while(1)
	{
		...
		if(game_flag_temp == 1){return 0;}     //返回
		if(game_flag_temp == 2){void DinoGame_Pos_Init();DinoGame_Animation();}
