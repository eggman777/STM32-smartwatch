...
void Change_RTC_Time(uint8_t i,uint8_t flag)  //flag=1，加一；flag=0，减一
{
...
int SetHour(void)
{
	while(1)
	{
		...
		{
			Change_RTC_Time(3,1);
			if(MyRTC_Time[3]>=24){MyRTC_Time[3]=1;MyRTC_SetTime();}
		}
		else if(KeyNum==2)//数值减一
		{
			Change_RTC_Time(3,0);
...
int SetMin(void)
{
	while(1)
	{
		...
		{
			Change_RTC_Time(4,1);
			if(MyRTC_Time[4]>=60){MyRTC_Time[4]=1;MyRTC_SetTime();}
		}
		else if(KeyNum==2)//数值减一
		{
			Change_RTC_Time(4,0);
...
int SetSec(void)
{
	while(1)
	{
		...
		{
			Change_RTC_Time(5,1);
			if(MyRTC_Time[5]>=60){MyRTC_Time[5]=1;MyRTC_SetTime();}
		}
		else if(KeyNum==2)//数值减一
		{
			Change_RTC_Time(5,0);
